# from gym_splendor.envs.agents import agent_random as a1
# from gym_splendor.envs.agents import agent_random as a2
# from gym_splendor.envs.agents import agent_random as a3
# from gym_splendor.envs.agents import agent_random as a4

from gym_splendor.envs.agents import random_final as a1
from gym_splendor.envs.agents import random_final as a2
from gym_splendor.envs.agents import random_final as a3
from gym_splendor.envs.agents import Long as a4
from gym_splendor.envs.agents import Long1 as a5

# ListPlayer = [a1.Agent("Trang"),a2.Agent("Trang2"),a3.Agent("Dat"),a4.Agent("longlee")]
ListPlayer = [a1.Agent("final1"),a2.Agent("final2"),a3.Agent("final3"),a4.Agent("Long")]
